...
export function appReducer(state: AppState, action: Action): AppState {
    ...
    if (isAction(playback.SEEK)) {
        const playAction = <playback.SeekAction> action;
        return extend({playSince: playAction.time});
    }

    if (isAction(playback.UPDATE_PROGRESS)) {
        const updateProgressAction = <playback.UpdateProgressAction> action;
        return extend({
            currentTime: updateProgressAction.currentTime,
            totalTime: updateProgressAction.totalTime
        });
    }
